  ...
  <v-card class="main-card">
    ...
    <v-dialog v-model="dialog" max-width="500">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn @click="dialog = false" dark depressed color="#43B581">
            {{ $t("OK") }}
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="mr-2" @click="share(item)"> mdi-share </v-icon>
      </template>
          ...
          >
            <div v-if="!item.multiplayer">
              <div :key="index" v-for="(r, index) in item.rounds">
                ...
                <GmapMarker :icon="icon" :position="r.position" />
              </div>
            </div>
            <div v-else>
      ...
      this.url =
        ...
        btoa(
          [
            item.difficulty,
            item.timeLimitation,
            item.rounds.map((r) => r.position.lat + "," + r.position.lng),
          ]
            .flat()
            .join(",")
        );
